...
const EventListing = () => {
  ...
  const [, setActiveFilters] = useState<{ sort: string; dateRange: string; location: string; priceRange: number[]; eventType: string } | null>(null);
  
  const categories = [
    'All', 'Tech', 'Art', 'Concerts', 'Parties', 'Culture', 'Business', 'Food & Drinks', 'Dating'
  ];
  ...
  const handleApplyFilters = (filters: { sort: string; dateRange: string; location: string; priceRange: number[]; eventType: string }) => {
    setActiveFilters(filters);
    setShowFilter(false);
  };

    return (
        <div className="mb-24 px-24 py-2">
          <div className="relative mb-8 flex items-center justify-between pb-2">
            <button className="flex flex-row items-center gap-2 rounded-md border border-[#F2F3F5] p-2 font-inter text-[14px] font-medium text-[#374252]" onClick={() => setShowFilter(!showFilter)}>
            ...
            {paginatedEvents.map(event => (
              <div key={event.id} className="group">
                <Link href={`/event/${event.id}`}>
                  <div className="relative mb-2 overflow-hidden rounded-lg bg-gray-200">
                  ...
                  <div className='flex flex-col gap-2'>
                    ...
                    <h3 className="w-full overflow-hidden truncate whitespace-nowrap font-inter text-[16px] font-semibold text-[#001433]">{event.title}</h3>
                    <div className="font-inter text-sm font-medium text-[#6B7380]">{event.date}</div>
                    <div className="font-inter text-sm font-medium text-[#6B7380]">{event.location}</div>
                  </div>
                </Link>
              </div>
            ))}
          ...
          <div className="mt-12 flex items-center justify-center space-x-2">
            ...
            {[...Array(pagesCount)].map((_, i) => {
              ...
              if (!shouldShow && pageNum === currentPage + 2) {
                return <span key={`ellipsis-after`} className="px-3 py-2">...</span>;
              }
              
              if (!shouldShow && pageNum === currentPage - 2) {
                return <span key={`ellipsis-before`} className="px-3 py-2">...</span>;
              }
