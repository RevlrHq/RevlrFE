...
import { MediaProviderAuthState, ProviderResult } from '@/types/media-search';
...
export interface UnsplashAuthHookReturn {
    ...
    getUserPhotos: (page?: number, perPage?: number) => Promise<ProviderResult>;
...
export function useUnsplashAuth(): UnsplashAuthHookReturn {
    const [authState, setAuthState] = useState<MediaProviderAuthState>({
    ...
    const getUnsplashProvider =
        useCallback(async (): Promise<UnsplashProvider | null> => {
            try {
                const mediaService =
                    await MediaSearchServiceFactory.getInstance();
                const providers = mediaService.getAvailableProviders();
                const unsplashProvider = providers.find(
                    (p): p is UnsplashProvider => p.id === 'unsplash'
                );
    ...
    const updateAuthState = useCallback(async () => {
        const provider = await getUnsplashProvider();
        if (provider) {
    ...
    const likePhoto = useCallback(
        async (photoId: string) => {
            ...
            if (!provider) {
                return {
                    success: false,
                    error: 'Unsplash provider not available',
                };
            }
    ...
    const getUserLikedPhotos = useCallback(
    ...
    const getAvailableScopes = useCallback(async (): Promise<string[]> => {
        const provider = await getUnsplashProvider();
