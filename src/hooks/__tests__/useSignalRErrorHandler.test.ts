...
describe('useSignalRErrorHandler', () => {
    beforeEach(() => {
        jest.clearAllMocks();
    ...
    describe('Recovery Strategy Selection', () => {
        ...
        it('should select USER_INTERVENTION after max retry attempts', () => {
            ...
            act(() => {
                result.current.handleError(createTestError('network'));
            });
            act(() => {
                result.current.handleError(createTestError('network'));
            });
    ...
    describe('Error Handling', () => {
        ...
        it('should schedule automatic recovery for recoverable errors', async () => {
            const { result } = renderHook(() =>
                useSignalRErrorHandler({ retryIntervals: [100] })
            );

            const recoverableError = createTestError(
                'network',
                'Network error'
            );
            ...
            expect(result.current.errorState.currentError).toEqual(
                recoverableError
            );
    ...
    describe('Recovery Attempts', () => {
        ...
        it('should track recovery attempts', async () => {
            ...
            const { result } = renderHook(() =>
                useSignalRErrorHandler({ onRecoveryAttempt })
            );
            ...
            expect(result.current.errorState.recoveryAttempts).toHaveLength(1);
            expect(result.current.errorState.recoveryAttempts[0].strategy).toBe(
                RecoveryStrategy.RETRY_WITH_BACKOFF
            );
    ...
    describe('Error Frequency Tracking', () => {
        it('should track error frequency correctly', async () => {
            ...
            await act(async () => {
                await result.current.handleError(
                    createTestError('hub_method', 'Error 3')
                );
            });

            const frequency = result.current.getErrorFrequency();
            expect(frequency).toBe(3);
        });

        it('should exclude old errors from frequency calculation', async () => {
            const { result } = renderHook(() => useSignalRErrorHandler());
            ...
            await act(async () => {
                await result.current.handleError(
                    createTestError('network', 'Old error')
                );
            });

            // Advance time by more than 1 minute
            Date.now = jest.fn(() => baseTime + 70000);

            await act(async () => {
                await result.current.handleError(
                    createTestError('connection', 'New error')
                );
            });

            const frequency = result.current.getErrorFrequency();
            expect(frequency).toBe(1); // Only the new error should count
    ...
    describe('Configuration Options', () => {
        ...
        it('should use custom retry intervals', async () => {
            const customIntervals = [50, 100];
            const { result } = renderHook(() =>
                useSignalRErrorHandler({ retryIntervals: customIntervals })
            );
            ...
            // The error should schedule recovery with the first interval (50ms)
            expect(result.current.errorState.currentError).toEqual(error);
